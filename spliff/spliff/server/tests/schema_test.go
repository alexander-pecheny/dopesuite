...
// against testdata/schema.sql. Regenerate with SPLIFF_UPDATE_SCHEMA=1.
func TestSchemaIsPinned(t *testing.T) {
	path := filepath.Join(t.TempDir(), "fresh.db")
	db, err := spliffserver.OpenDB(path)
	if err != nil {
		t.Fatal(err)
	}
	...
	if os.Getenv("SPLIFF_UPDATE_SCHEMA") != "" {
	...
	if err != nil {
		t.Fatalf("%v — run with SPLIFF_UPDATE_SCHEMA=1 to write it", err)
	}
	if string(want) != fresh {
		t.Errorf("schema differs from testdata/schema.sql; if the change is meant, rerun with SPLIFF_UPDATE_SCHEMA=1\n%s",
			diffLines(string(want), fresh))
	}
