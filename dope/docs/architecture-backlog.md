...
deepening candidates. Nine are done (four on `dope-brain`, #7, #6, #5, #9
and #8 on `dope-refactor`); what is open is #10, with a note in this
directory written for a fresh agent. The vocabulary is
...
| 8′ | `replay.StandingsReader` and the `[таблица s1/g3]` transcript section: 27 tables in the studchr transcripts, checked against `stage_standings`; found and fixed a boundary reseed of winners only, flat ranks shared on бой place, and ЭК's пересев rule | — |
...
Not urgent: only if the grid changes anyway — #7 rebuilt the group table as
a бой box, so a planner would now front one box kind, not two.
