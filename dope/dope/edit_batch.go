...
// Editor-side write batching. Every game-state PATCH used to take the global
// write mutex, run its own transaction and fsync-commit on its own — so N
// concurrent editors hammering a grid produced N lock acquisitions and N
// commits per burst, the dominant source of write contention (and the shape of
// the 2026-06-13 freeze, where one stuck write pinned the lock for everyone).
//
// editBatcher mirrors the viewer-side delta coalescing (deltaCoalesceWindow) on
// the WRITE side: edits to one game within editBatchWindow are collected and
// applied together under a SINGLE lock acquisition in ONE transaction with ONE
...
type editJob struct {
	scope    festScope
	req      gameStatePatchRequest
	payload  string          // raw request body, recorded verbatim in the journal
	ctx      context.Context // detached audit context (actor/request/fest/game), writeTxTimeout-bound
	cancel   context.CancelFunc
	sample   *editSample // optional metrics, nil when off
	submitAt time.Time   // set only when metrics on, for the wait breakdown
...
// editBatcher holds the open (still-collecting) batch per game id.
...
func (s *server) submitEdit(reqCtx context.Context, scope festScope, req gameStatePatchRequest, payload string, sample *editSample) ([]byte, int64, error) {
	// Detached, attribution-carrying context so the write stays attributed to the
	// acting user/request and is bounded by writeTxTimeout, and is NOT aborted by
	...
	auditCtx, cancel := auditDetachedContext(reqCtx, scope.FestID)
	job := &editJob{
		scope:   scope,
		req:     req,
		payload: payload,
...
func (s *server) enqueueEdit(job *editJob) {
	b := &s.editBatcher
	b.mu.Lock()
	if b.pending == nil {
...
func (s *server) flushEditBatch(gameID int64) {
	b := &s.editBatcher
	b.mu.Lock()
...
// hang forever.
func (s *server) applyEditBatch(batch *editBatch) {
	scope := batch.scope
	jobs := batch.jobs
	defer func() {
		if r := recover(); r != nil {
			log.Printf("recovered panic in edit batch flush for game %d: %v", scope.GameID, r)
			s.failBatch(jobs, fmt.Errorf("internal error applying edit batch"))
		}
	}()

	metrics := s.editMetricsOn

	// Acquire the dedicated connection OFF the global lock (pool wait bounded by
	// writeTxTimeout), exactly as the hardened match-write path does, so a starved
	// pool can never pin s.mu. See the 2026-06-13 freeze note in audit.go.
	txCtx, cancelTx := context.WithTimeout(context.Background(), writeTxTimeout)
	defer cancelTx()
	conn, err := s.acquireWriteConn(txCtx, "edit-batch")
	if err != nil {
		s.failBatch(jobs, err)
		return
	}
	defer conn.Close()

	if metrics {
		s.writeWaiters.Add(int64(len(jobs)))
	}
	unlock := s.lockWrite("edit-batch")
	acquired := time.Now()
	if metrics {
		s.writeWaiters.Add(-int64(len(jobs)))
	}
	...
	for i, job := range jobs {
		...
		_ = seedAuditCtx(job.ctx, tx)
		next, revision, opsJSON, err := s.applyOneEditTx(job.ctx, tx, scope, job.req, job.payload, job.sample)
		if err != nil {
	...
	if !committedAny {
		...
		unlock()
		for i, job := range jobs {
			s.deliver(job, results[i], metrics, acquired, 0)
		}
		return
	}
	...
	for i, job := range jobs {
		s.deliver(job, results[i], metrics, acquired, hold)
	}
...
// detached context.
func (s *server) deliver(job *editJob, res editResult, metrics bool, acquired time.Time, hold time.Duration) {
	if metrics && job.sample != nil {
...
func (s *server) failBatch(jobs []*editJob, err error) {
...
func (s *server) applyOneEditTx(ctx context.Context, tx *sql.Tx, scope festScope, req gameStatePatchRequest, payload string, sample *editSample) ([]byte, int64, []byte, error) {
	if len(req.Ops) == 0 {
...
select game_type, state_json from games where fest_id = ? and id = ?`,
	...
	for _, op := range req.Ops {
		...
		if patchPathTouchesRatingRoster(gameType, path) {
			return nil, 0, nil, errRatingRosterImmutable
		}
