...
func (s *server) authorizeBot(w http.ResponseWriter, r *http.Request) bool {
	...
	if subtle.ConstantTimeCompare([]byte(got), []byte(s.botSecret)) != 1 {
...
func (s *server) handleTelegramRegister(w http.ResponseWriter, r *http.Request) {
	...
	writeJSONValue(w, telegramBridgeResponse{Message: msg})
}
...
func (s *server) telegramIssueLogin(ctx context.Context, tgUserID int64, tgUsername string) string {
	...
	defer s.mu.Unlock()
	var userID int64
	err := s.db.QueryRowContext(ctx, `select id from users where telegram_user_id = ? and is_system = 0`, tgUserID).Scan(&userID)
	if errors.Is(err, sql.ErrNoRows) {
