// Package overrides holds the host player→team override UI data layer: the view
// types, the read helpers that build the override options/rows, and the
// save/replace write operations. Pure store reads plus a Host{WithWriteTx} for
// the writes — no server coupling — so the host page handlers and the server
// share one definition. (Game-roster materialization and rating-override restore
// stay in dopeserver, which uses these types + roster.EnsureSeed*.)
package overrides
...
func LoadHostPlayerOverrideOptions(ctx context.Context, db store.Queryer, festID int64) ([]HostPlayerOverrideOption, []HostTeamOverrideOption, []HostGameOverrideOption, []HostPlayerOverrideRow, error) {
	players, err := loadHostPlayerOverridePlayerOptions(ctx, db, festID)
	if err != nil {
		return nil, nil, nil, nil, err
	}
	teams, err := loadHostPlayerOverrideTeamOptions(ctx, db, festID)
	if err != nil {
		return nil, nil, nil, nil, err
	}
	games, err := LoadHostPlayerOverrideGameOptions(ctx, db, festID)
	if err != nil {
		return nil, nil, nil, nil, err
	}
	rows, err := LoadHostPlayerOverrideRows(ctx, db, festID)
	if err != nil {
		return nil, nil, nil, nil, err
	}
...
func SavePlayerTeamOverride(h Host, reqCtx context.Context, festID, playerID, overrideTeamID int64, gameIDs []int64) (int64, []int64, error) {
	...
	var revision int64
	var ekGameIDs []int64
	err := h.WithWriteTx(reqCtx, festID, "player-override-save", func(ctx context.Context, tx *sql.Tx) error {
...
on conflict(fest_id, game_id, player_id) do update set
  ...
  updated_at = excluded.updated_at`,
		...
		revision, err = festwrite.BumpFestRevisionTx(ctx, tx, festID, "roster:player-override", util.MustJSON(map[string]any{
			"playerID":       playerID,
...
func ReplacePlayerTeamOverride(h Host, reqCtx context.Context, festID, playerID, sourceTeamID, overrideTeamID int64, gameIDs []int64) (int64, []int64, error) {
	...
	var revision int64
	var ekGameIDs []int64
	err := h.WithWriteTx(reqCtx, festID, "player-override-replace", func(ctx context.Context, tx *sql.Tx) error {
...
values(?, ?, ?, ?, ?, ?, ?)`,
		...
		revision, err = festwrite.BumpFestRevisionTx(ctx, tx, festID, "roster:player-override-edit", util.MustJSON(map[string]any{
			"playerID":       playerID,
			"sourceTeamID":   sourceTeamID,
			"overrideTeamID": overrideTeamID,
...
values(?, ?, ?, ?)`, gameID, teamID, playerID, rosterOrder); err != nil {
	...
	_, err = tx.ExecContext(ctx, `update games set roster_source = 'game', updated_at = ? where id = ? and fest_id = ?`, util.UtcNow(), gameID, festID)
	return err
}
