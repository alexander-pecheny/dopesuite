...
func hostTeamsDoc(data hostFestRosterData) *dopeui.Doc {
	...
	page := []dopeui.Item{
		...
		dopeui.Publictopbar(pages.Trail(pages.FestCrumbs(data.Fest.HostBase(), data.Fest.Title), s.Host.Roster.TeamsCrumb())),
	}
	if len(data.Teams) > 0 {
...
func hostPlayersDoc(data hostFestRosterData) *dopeui.Doc {
	...
	page := []dopeui.Item{
		...
		dopeui.Publictopbar(pages.Trail(pages.FestCrumbs(data.Fest.HostBase(), data.Fest.Title), s.Host.Roster.PlayersCrumb())),
	}
	if data.Error != "" {
...
func hostRatingImportDoc(data hostFestImportData) *dopeui.Doc {
	...
	page := []dopeui.Item{
		...
		dopeui.Publictopbar(pages.Trail(pages.FestCrumbs(data.Fest.HostBase(), data.Fest.Title), s.Host.Roster.RatingImportCrumb())),
	}
...
func hostSchemeImportDoc(data hostFestImportData) *dopeui.Doc {
	...
	page := []dopeui.Item{
		...
		dopeui.Publictopbar(pages.Trail(pages.FestCrumbs(data.Fest.HostBase(), data.Fest.Title), s.Host.Roster.SchemeImportCrumb())),
	}
	page = append(page, importMessages(data.Error, data.Notice)...)
