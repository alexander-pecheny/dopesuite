...
from journal where fest_id = ? and op = ? order by seq desc limit 1`,
	...
	evs, err := srv.Eng().JournalEventsSince(ctx, festID, 0)
	if err != nil {
	...
	if _, err := journal.ArchiveFest(ctx, srv.Eng().DB, festID, lastSeq); err != nil {
	...
	evs2, err := srv.Eng().JournalEventsSince(ctx, festID, 0)
	if err != nil {
