...
values(?, '2026-01-01T00:00:00.000Z', 'widgets', ?, ?, ?, ?, 7, 'req1', 1)`,
	...
	rep, err := migrate.ConvertAuditLog(db)
	if err != nil {
...
func TestConvertReplayEquivalenceRealDB(t *testing.T) {
	...
	if _, err := migrate.ConvertAuditLog(db); err != nil {
		t.Fatalf("convert: %v", err)
	}
	dict, err := journal.LoadDict(ctx, db)
	if err != nil {
	...
	for rows.Next() {
		...
		switch op {
		case "INSERT", "UPDATE":
			want, _ = migrate.DecodeRowJSON(aj.String)
		case "DELETE":
			// only pk columns are kept; got must be a subset of the full row
			want, _ = migrate.DecodeRowJSON(bj.String)
		}
