...
func TestSavingAFrozenVotingKeepsItsCandidates(t *testing.T) {
	...
	if err := inTx(t, db, func(ctx context.Context, tx *sql.Tx) error {
		return venues.SaveVotingTx(ctx, tx, slot.ID, venues.KindAny, false, "", "2026-09-05 12:00", nil)
	}); err != nil {
	...
	if saved.ClosesAt != "2026-09-05 12:00" || saved.Kind != venues.KindAny || saved.PerTeam {
