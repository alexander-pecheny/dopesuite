...
func scopedAPITestIDs(t *testing.T, srv *dopeserver.Server) (int64, int64) {
	t.Helper()
	var festID int64
	if err := srv.Eng().DB.QueryRow(`select id from fests order by id limit 1`).Scan(&festID); err != nil {
		t.Fatalf("fest id: %v", err)
	}
	gameID, err := dopeserver.DefaultGameID(t.Context(), srv.Eng().DB, festID)
	if err != nil {
...
func createAPITestSession(t *testing.T, srv *dopeserver.Server, username string) (int64, string) {
	t.Helper()
	tx, err := srv.Eng().DB.BeginTx(t.Context(), nil)
	if err != nil {
...
values(null, null, ?, 0, ?, ?)`, username, now, now)
	...
	token, err := dopeserver.CreateSessionTx(t.Context(), tx, userID, time.Now().UTC())
	if err != nil {
...
func addAPITestOrganizer(t *testing.T, srv *dopeserver.Server, festID, userID int64) {
	t.Helper()
	addAPITestRole(t, srv, festID, userID, roles.Admin)
}

func addAPITestRole(t *testing.T, srv *dopeserver.Server, festID, userID int64, role string) {
	t.Helper()
	if _, err := srv.Eng().DB.Exec(`
insert into fest_organizers(fest_id, user_id, role, added_at)
values(?, ?, ?, ?)
on conflict(fest_id, user_id) do update set role = excluded.role`, festID, userID, role, util.UtcNow()); err != nil {
		t.Fatalf("add role %s: %v", role, err)
	}
}

func scopedAPIRequest(t *testing.T, srv *dopeserver.Server, method, path string, body any, token string) *httptest.ResponseRecorder {
	t.Helper()
...
func TestHostRoleCanEditGameTablesOnly(t *testing.T) {
	...
	venueResp := scopedAPIRequest(t, srv, http.MethodPut, fmt.Sprintf("/api/fest/%d/venues/1", festID), dopeserver.VenueUpdateRequest{Title: "Новая"}, hostToken)
	...
	importReq.AddCookie(&http.Cookie{Name: session.CookieName, Value: hostToken})
...
func TestScopedAPIImportRequiresFestOrganizer(t *testing.T) {
	...
	scheme := store.FestScheme{
		...
		Stages: []store.SchemeStage{{
			...
			Matches: []store.SchemeMatch{{
				Code:             dopeserver.DefaultMatchCode,
				Title:            "A",
				ParticipantCount: 1,
	...
	nonOrganizerReq.AddCookie(&http.Cookie{Name: session.CookieName, Value: nonOrganizerToken})
	...
	organizerReq.AddCookie(&http.Cookie{Name: session.CookieName, Value: organizerToken})
	...
	manualTeamsReq.AddCookie(&http.Cookie{Name: session.CookieName, Value: organizerToken})
...
func TestScopedMatchUpdateResponseCarriesBroadcastSeq(t *testing.T) {
	...
	matchScope := fmt.Sprintf("match:%d:%s", gameID, dopeserver.DefaultMatchCode)

	firstView := edit(0)
	srv.Eng().FlushDelta(matchScope)
