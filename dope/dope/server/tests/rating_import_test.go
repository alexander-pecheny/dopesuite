...
func TestImportRatingRosterRemapsKSIScoresByTeam(t *testing.T) {
	...
	next := []roster.FestRosterImportTeam{
		{RatingID: 3, Name: "Гамма"},
		{RatingID: 5, Name: "Эхо"},
...
func TestImportRatingRosterPropagatesFlags(t *testing.T) {
	...
	if _, err := imports.ImportFestRoster(srv.Eng(), t.Context(), festID, 13533, teams); err != nil {
	...
	result, err := imports.ImportFestRoster(srv.Eng(), t.Context(), festID, 13533, teams)
	if err != nil {
		t.Fatalf("re-import: %v", err)
	}
	if result.Unchanged {
