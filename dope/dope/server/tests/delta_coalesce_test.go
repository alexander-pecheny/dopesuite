...
func TestBroadcastStateDeltaCoalescesForViewersImmediateForEditors(t *testing.T) {
	...
	srv.Eng().RT.AddSubscriber(1, editor, true, 0)
	srv.Eng().RT.AddSubscriber(1, viewer, false, 0)

	scope := "game-state:5"
	seq1 := srv.Eng().BroadcastStateDelta(1, scope, 10, []byte(`[{"op":"set","path":["x"],"value":1}]`))
	seq2 := srv.Eng().BroadcastStateDelta(1, scope, 11, []byte(`[{"op":"set","path":["y"],"value":2}]`))
...
func TestBroadcastStateFlushesBufferedDeltasFirst(t *testing.T) {
	...
	srv.Eng().RT.AddSubscriber(1, ch, false, 0)
	scope := "game-state:5"

	srv.Eng().BroadcastStateDelta(1, scope, 10, []byte(`[{"op":"set","path":["x"],"value":1}]`))
	snapSeq := srv.Eng().BroadcastState(1, scope, 11, []byte(`{"full":true}`))
