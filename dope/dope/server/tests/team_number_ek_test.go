...
func TestEnsureSeedTeamByNumberKeysByNumber(t *testing.T) {
	...
	id7, _, err := imports.EnsureSeedTeamByNumber(ctx, tx, festID, 7, "Дубль", "Москва", nil)
	if err != nil {
		t.Fatalf("seed #7: %v", err)
	}
	id8, _, err := imports.EnsureSeedTeamByNumber(ctx, tx, festID, 8, "Дубль", "Питер", nil)
	if err != nil {
	...
	id7b, _, err := imports.EnsureSeedTeamByNumber(ctx, tx, festID, 7, "Дубль-2", "", nil)
	if err != nil {
	...
	id0, _, err := imports.EnsureSeedTeamByNumber(ctx, tx, festID, 0, "Без номера", "", nil)
	if err != nil {
...
func TestBackfillEKTeamNumbers(t *testing.T) {
	...
	if err := dopeserver.BackfillEKTeamNumbers(db); err != nil {
