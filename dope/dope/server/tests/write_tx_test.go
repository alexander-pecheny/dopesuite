...
func TestWithWriteTxCommitAndRollback(t *testing.T) {
	...
	if err := srv.Eng().WithWriteTx(ctx, festID, "test-commit", func(ctx context.Context, tx *sql.Tx) error {
	...
	var title string
	if err := srv.Eng().DB.QueryRow(`select title from fests where id = ?`, festID).Scan(&title); err != nil {
		t.Fatal(err)
	}
	if title != "committed" {
	...
	err := srv.Eng().WithWriteTx(ctx, festID, "test-rollback", func(ctx context.Context, tx *sql.Tx) error {
