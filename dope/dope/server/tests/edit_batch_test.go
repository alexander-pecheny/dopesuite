...
func festRevision(t *testing.T, srv *dopeserver.Server, festID int64) int64 {
	t.Helper()
	var rev int64
	if err := srv.Eng().DB.QueryRow(`select revision from fests where id = ?`, festID).Scan(&rev); err != nil {
...
func TestEditBatchCoalescesConcurrentEdits(t *testing.T) {
	...
	seqBefore := srv.Eng().CurrentStateSeq(scopeKey)
	...
	if got := srv.Eng().CurrentStateSeq(scopeKey) - seqBefore; got != 1 {
