...
func TestScopedGameStateRejectsUnnumberedTeams(t *testing.T) {
	...
	if _, err := srv.Eng().DB.Exec(`delete from fest_teams where fest_id = ?`, festID); err != nil {
...
values(?, 201, 'Без номера', '', 1, null, 0)`, festID); err != nil {
	...
	if _, err := srv.Eng().DB.Exec(`update fest_teams set number = 1 where fest_id = ? and rating_id = 201`, festID); err != nil {
