...
func seedRegisterCode(t *testing.T, s *server, code string, expires time.Time) {
	t.Helper()
...
func seedTelegramUser(t *testing.T, s *server, tgUserID int64, username string) {
	t.Helper()
	now := time.Now().UTC().Format(time.RFC3339)
	if _, err := s.db.Exec(`
...
func TestTelegramBridgeSecretGate(t *testing.T) {
	...
	// Missing header -> 401.
	rec = httptest.NewRecorder()
	s.handleTelegramLogin(rec, httptest.NewRequest(http.MethodPost, "/api/telegram/login", strings.NewReader(`{"telegram_user_id":1}`)))
	if rec.Code != http.StatusUnauthorized {
...
func TestTelegramBridgeRegisterHandler(t *testing.T) {
	...
	s.handleTelegramRegister(rec, req)

	if rec.Code != http.StatusOK {
		t.Fatalf("register = %d, want 200 (%s)", rec.Code, rec.Body.String())
	}
	var resp telegramBridgeResponse
	if err := json.Unmarshal(rec.Body.Bytes(), &resp); err != nil {
		t.Fatalf("decode: %v", err)
	}
	if resp.Message != telegramBridgeRegisterSuccess {
	...
	var tgID int64
	var consumed string
	if err := s.db.QueryRow(`select telegram_user_id, consumed_at from telegram_login_codes where code = 'ABCD2345'`).Scan(&tgID, &consumed); err != nil {
...
func TestTelegramBridgeConsumeRegisterReasons(t *testing.T) {
	...
	// Unknown code.
	if got := s.telegramConsumeRegister(ctx, "ZZZZ2345", 1, "x"); got != telegramBridgeCodeMissing {
		t.Fatalf("unknown = %q, want missing", got)
	}
	// Non-code shape -> missing (never hits the DB).
	if got := s.telegramConsumeRegister(ctx, "!!", 1, "x"); got != telegramBridgeCodeMissing {
		t.Fatalf("bogus = %q, want missing", got)
	}
	// Expired.
	seedRegisterCode(t, s, "EXPIRED2", time.Now().Add(-time.Minute))
	if got := s.telegramConsumeRegister(ctx, "EXPIRED2", 1, "x"); got != telegramBridgeCodeExpired {
		t.Fatalf("expired = %q, want expired", got)
	}
	// Success then already-consumed.
	seedRegisterCode(t, s, "FRESH234", time.Now().Add(time.Minute))
	if got := s.telegramConsumeRegister(ctx, "FRESH234", 5, "y"); got != telegramBridgeRegisterSuccess {
		t.Fatalf("first consume = %q, want success", got)
	}
	if got := s.telegramConsumeRegister(ctx, "FRESH234", 5, "y"); got != telegramBridgeCodeConsumed {
...
func TestTelegramBridgeIssueLogin(t *testing.T) {
	...
	if got := s.telegramIssueLogin(ctx, 9999, "ghost"); got != telegramBridgeLoginNeedInvite {
		t.Fatalf("unknown user = %q, want need-invite", got)
	}

	seedTelegramUser(t, s, 4242, "bob")
	msg := s.telegramIssueLogin(ctx, 4242, "bob")
	if !strings.Contains(msg, "<code>") {
		t.Fatalf("issue login = %q, want a code", msg)
	}
	// A login code row must now exist for the user.
	var n int
	if err := s.db.QueryRow(`select count(*) from telegram_login_codes where kind='login' and telegram_user_id=4242 and consumed_at is null`).Scan(&n); err != nil {
