...
// matchDeltaSetOp is one "set value at path" operation in a scoped delta. It
// matches the shape the client applies (static/match-table.js applySetPatch):
// path segments are object keys (string) or array indices (number), and value
// is the new JSON subtree to assign there.
type matchDeltaSetOp struct {
	Op    string          `json:"op"`
	Path  []any           `json:"path"`
...
func diffJSONInto(path []any, oldRaw, newRaw json.RawMessage, ops *[]matchDeltaSetOp) {
	...
	if json.Unmarshal(oldRaw, &oldObj) == nil && json.Unmarshal(newRaw, &newObj) == nil &&
		...
		for k, nv := range newObj {
			...
			if ov, ok := oldObj[k]; ok {
				...
				*ops = append(*ops, matchDeltaSetOp{Op: "set", Path: child, Value: nv})
			}
		}
		for k := range oldObj {
			if _, ok := newObj[k]; !ok {
				*ops = append(*ops, matchDeltaSetOp{Op: "set", Path: appendPath(path, k), Value: json.RawMessage("null")})
			}
		}
		return
	}
	...
	*ops = append(*ops, matchDeltaSetOp{Op: "set", Path: appendPath(path, nil), Value: newRaw})
}
