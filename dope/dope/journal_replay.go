...
// Row-op payload (live triggers + derived revert) is a small self-describing
// JSON document: {"t":"<table>","r":{<columns>}}. (Cold segments from the
// audit converter use the compact varint rowArgs instead; both decode to the
// same (table, row) and feed applyRowMap.) JSON keeps the trigger SQL simple
// (json_object) and stays debuggable; zstd on cold segments reclaims the size.
...
func (rp *journalReplayer) applyRowMap(ctx context.Context, tx interface {
	pkQuerier
	execer
}, op journalOp, table string, row map[string]any) error {
	switch op {
	case opRowIns:
...
// journalReplayer applies forward journal records to a database to reconstruct
// state. Replaying a checkpoint's records (or a whole stream from genesis)
// rebuilds the rows as they were at any point — this is the foundation for both
// historical inspection and derived revert.
//
// For now it understands the generic row opcodes (the converter's output and
// the coarse fallback). Semantic opcodes register their own appliers here in a
// later phase; an unknown opcode is a hard error so a gap can never be silently
// skipped (the integrity canary depends on this).
...
func newJournalReplayer(dict map[uint64]string) *journalReplayer {
	return &journalReplayer{dict: dict, pksBy: map[string][]string{}}
}

// loadJournalDict reads the journal_dict table into an id->string map.
...
func (rp *journalReplayer) tablePKs(ctx context.Context, tx pkQuerier, table string) ([]string, error) {
...
func (rp *journalReplayer) rowFromArgs(rec journalRecord) (table string, row map[string]any, err error) {
...
func (rp *journalReplayer) apply(ctx context.Context, tx interface {
	pkQuerier
	execer
}, rec journalRecord) error {
	switch rec.Op {
...
func (rp *journalReplayer) applyRowOp(ctx context.Context, tx interface {
	pkQuerier
	execer
}, rec journalRecord) error {
...
func (rp *journalReplayer) applyAll(ctx context.Context, tx interface {
	pkQuerier
	execer
}, recs []journalRecord) error {
	for i := range recs {
...
func replayDelete(ctx context.Context, tx execer, table string, pks []string, row map[string]any) error {
	...
	keyCols := pks
	if len(keyCols) == 0 {
		keyCols = sortedKeys(row)
	}
	where, whereArgs, err := pkWhere(keyCols, row)
	if err != nil {
		return err
	}
