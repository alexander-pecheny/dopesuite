...
// `<number><tab or whitespace><name>`; the number must be 1..festNumberMax.
...
func parseNumberImport(text string) (entries []importEntry, errs []string) {
	...
	for _, raw := range strings.Split(text, "\n") {
		...
		if err != nil || n <= 0 || n > festNumberMax {
			errs = append(errs, "Строка "+strconv.Itoa(lineNo)+": номер должен быть целым от 1 до "+strconv.Itoa(festNumberMax)+".")
			continue
		}
...
func matchNumberImport(entries []importEntry, teams []festNumberingTeam) []importMatch {
	type cand struct {
		teamID   int64
		distance int
	}
	...
	for ei, e := range entries {
		...
		for ti, t := range teams {
			...
			if dc := levenshtein(norm, normalizeTeamName(teamDisplayName(t))); dc < d {
				d = dc
			}
...
func (s *server) handleHostFestNumbersImportMatch(w http.ResponseWriter, r *http.Request, festID int64) {
	...
	for _, t := range sorted {
		options = append(options, importTeamOption{ID: t.ID, Label: teamDisplayName(t)})
	}
	if matches == nil {
		matches = []importMatch{}
	}
	...
	writeJSONValue(w, importMatchResponse{Teams: options, Matches: matches, Errors: errs})
}
...
func (s *server) handleHostFestNumbersImportApply(w http.ResponseWriter, r *http.Request, festID int64) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		writeJSONValue(w, importApplyResponse{Error: "Не удалось прочитать данные."})
		return
	}
	teams, err := loadFestTeamsForNumbering(r.Context(), s.db, festID)
	if err != nil {
	...
	for _, a := range req.Assignments {
		if !validIDs[a.TeamID] {
			writeJSONValue(w, importApplyResponse{Error: "Команда не из этого феста."})
			return
		}
		if a.Number <= 0 || a.Number > festNumberMax {
			writeJSONValue(w, importApplyResponse{Error: "Номер должен быть целым от 1 до " + strconv.Itoa(festNumberMax) + "."})
			return
		}
		if seenTeam[a.TeamID] {
			writeJSONValue(w, importApplyResponse{Error: "Команда выбрана несколько раз."})
			return
		}
	...
	if err := s.saveFestNumbers(r.Context(), festID, final); err != nil {
		writeJSONValue(w, importApplyResponse{Error: err.Error()})
		return
	}
	writeJSONValue(w, importApplyResponse{OK: true, Assigned: len(req.Assignments)})
}
