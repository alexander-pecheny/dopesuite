...
select id, coalesce(rating_id, 0), name, city, coalesce(country, ''), coalesce(number, 0)
from fest_teams
where fest_id = ? and deleted = 0
order by position, id`, []any{festID}, func(rows *sql.Rows) (loaded, error) {
		var row loaded
		if err := rows.Scan(&row.id, &row.team.RatingID, &row.team.Name, &row.team.City, &row.team.Country, &row.team.Number); err != nil {
			return row, err
		}
		return row, nil
	})
