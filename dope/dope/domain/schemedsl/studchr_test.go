...
// tournamentов и ни одного нового вида блока под них.

// EK: 48 команд, сетка боёв на четверых, из каждого проходят двое. Регламент
// играет четвертьфинал втроём и пересевает перед ним. Столов шесть, поэтому
// 1/16 финала идёт в два захода — как и было на tournamentе.
const studchrEKSrc = `
[defaults]
venues: 6
...
reseed: r3
stats_from: [s1]
sorting: [place_sum, total, plus, correct_50, correct_40, correct_30, correct_20]
...
// OD: командная викторина, шесть туров по пятнадцать вопросов, все шестьдесят
...
// KSI: командная своя игра, двадцать тем, все команды за одним столом.
const studchrKSISrc = `
[scheme]
title: КСИ
kind: flat
