...
// cookie for it.
func makeUserWithSession(t *testing.T, srv *server, username string) string {
	t.Helper()
...
func TestAdminCreateUsersHappyPath(t *testing.T) {
	...
	srv.handleAdminCreateUsers(resp, req)
...
values(null, null, 'anton', 0, ?, ?)`, now, now); err != nil {
	...
	var antonCount int
	if err := srv.db.QueryRow(`select count(*) from users where username = 'anton'`).Scan(&antonCount); err != nil {
...
func TestAdminPagesRejectNonAdmin(t *testing.T) {
	...
	for _, path := range []string{"/admin", "/admin/create_users"} {
		...
		resp := httptest.NewRecorder()
		if path == "/admin" {
			srv.handleAdminLanding(resp, req)
		} else {
			srv.handleAdminCreateUsers(resp, req)
		}
	...
	srv.handleAdminCreateUsers(postResp, postReq)
	if postResp.Code != http.StatusNotFound {
		t.Fatalf("non-admin POST status = %d, want 404", postResp.Code)
	}
	var sneaky int
	if err := srv.db.QueryRow(`select count(*) from users where username = 'sneaky'`).Scan(&sneaky); err != nil {
