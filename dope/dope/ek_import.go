...
// ek_import.go is a one-shot maintenance entrypoint
...
func runEKImport(args []string) {
	...
	for _, code := range plan.Order {
		if len(code) > 8 && code[0] == '@' {
			...
			if _, err := calculateReseedStageSlotsTx(ctx, tx, plan.GameID, stageCode); err != nil {
...
on conflict(match_id, team_id) do update set place = excluded.place`,
		...
		for _, th := range t.Themes {
			themeID, err := lookupThemeID(ctx, tx, match.MatchID, t.TeamID, "regular", th.ThemeIndex)
			if err != nil {
...
on conflict(theme_id, answer_index) do update set mark = excluded.mark`,
	...
	if _, err := resolveGameSlotsTx(ctx, tx, plan.GameID); err != nil {
		return err
	}
