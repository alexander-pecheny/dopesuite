...
| `multi.ts` | ~330 | Мультиигры page — a column block per мини-игра with its subtotal, Итог last, Σ+ only where a мини-игра can take points away; numeric cells that cycle through a small domain and are typed into a wide one |
| `troika.ts` | ~590 | Тройка page — бои of three chair rows per side across темы of three вопросы, the «рассадка начиная с темы N» control, the группа tables and the Статистика fold |
| `crosstable.ts` | ~135 | The группа cross-table every two-seat format draws: who met whom, what the бой finished, and the block's standings columns beside it. Брейн and Тройка both read it; a format that adds a column names the metric |
| `troika-stats.ts` | ~120 | Тройка's Статистика fold: what a player took first apart from what they repeated after someone else had already said it, and the rate of the repeats they had a chance at |
