...
| `game-page.ts` | ~330 | Page plumbing — window-globals contract (init payloads, menu chrome), route parsing (both trees: a Fest under `/fest`, a Venue under `/venue`, each with its `/host` twin), the header breadcrumb trail (🏠 / [Мои фесты | Площадки] / фест / игра / раздел, mirroring the URL), menu jump/download mounts, localStorage snapshot cache, init/cache/fetch game-data loader |
