...
| `od-protocol.ts` · `ksi-protocol.ts` · `brain-protocol.ts` · `multi-protocol.ts` · `troika-protocol.ts` | ~330 · ~280 · ~35 · ~215 · ~135 | **A Protocol's document as the page reads it** (ADR-0018): the state shape, `parseState(raw, …)` — the adapter from whatever the server stored to what the renderers may trust (padded grids, normalised shootout rounds, the sticker grid, the two брейн sides) — and the arithmetic over it (ОД: `questionStats`, totals, tour sums, the rating, the shootout tiebreak, `placesFor`, `rows`; КСИ: `rulesOf`, `markContribution`, `computeThemeValue`, `scoreSheet`, `rankedResultRows`). Мультиигры: `rulesOf` (the мини-игры and their per-column domains), `scoreSheet`, `rankedResultRows`; Тройка: `parseState`, `questionScore` (every correct answer pays the тема's нарицательная on its own), `swapFrom`. Pure: every function takes the state it reads, so the pages call them with theirs and `jstest/*-protocol.test.js` with fixtures |
...
| `multi.ts` | ~370 | Мультиигры page — a column block per мини-игра with its subtotal, Итог last, Σ+ only where a мини-игра can take points away; numeric cells that cycle through a small domain and are typed into a wide one |
| `troika.ts` | ~645 | Тройка page — бои of three chair rows per side across темы of three вопросы, a «рассадка» column before every тема a side turned round at, the группа tables and the Статистика fold |
| `crosstable.ts` | ~165 | The группа cross-table every two-seat format draws: who met whom, what the бой finished, and the block's standings columns beside it, plus the `slotKey`/`crossSlot`/`standingsByParticipant` a caller needs to feed it. Брейн and Тройка both read it; a format that adds a column names the metric |
| `troika-stats.ts` | ~125 | Тройка's Статистика fold: what a player took first apart from what they repeated after someone else had already said it, and the rate of the repeats they had a chance at |
...
| `url-state.ts` | ~65 | Where a page keeps its place in the URL: `tabFromHash`/`setHashTab` (the hash names the tab, `canonicalKey` passed in for брейн's and Тройка's legacy hashes) and `param`/`setParam` (the query string carries what the viewer chose to look at). Every write is a `replaceState` rebuilding path + search + hash, so writing one half can never drop the other; `onNavigate` takes `hashchange` and `popstate` together. ОД, КСИ, брейн, Тройка and Мультиигры all read their tab through it |
| `divisions.ts` | ~80 | A Division (зачёт) as the viewer looks at one (ADR-0020): `divisionsOf` (every distinct Flag among a game's teams, first-seen order), `inDivision`, the `?division=` read/write pair, and `divisionChipRow` — the tab strip's own `.match-tab`, one chip per Division. The ranking itself stays in the two Protocol modules, which take the rows a Division holds |
