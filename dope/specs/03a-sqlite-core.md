...
| `tournaments` | `id`, `slug`, `title`, `description`, `rating_id?`, `created_by`, `revision`, `created_at`, `updated_at` |
| `tournament_organizers` | `tournament_id`, `user_id`, `added_at` |
| `games` | `id`, `tournament_id`, `code`, `title`, `game_type`, `position`, `scheme_id?`, `scheme_json`, `status`, `team_list_source`, `roster_source`, `revision`, `created_at`, `updated_at` |
| `teams` | `id`, `tournament_id`, `name`, `city` |
...
| `stages` | `id`, `tournament_id`, `game_id`, `code`, `title`, `stage_type`, `position`, `status`, `config_json` |
| `matches` | `id`, `tournament_id`, `game_id`, `stage_id`, `code`, `title`, `position`, `participant_count`, `venue_id?`, `status`, `revision` |
...
- `games`: уникальная пара `(tournament_id, code)`.
...
- `games.team_list_source` и `games.roster_source` — `'tournament'` (наследование) или `'game'` (override). При переключении на `'game'` система копирует текущее состояние турнира в `game_teams`/`game_team_players`. См. [09-tournaments-and-games.md](09-tournaments-and-games.md).
- `tournaments.created_by` и `tournament_organizers` решают, кто может редактировать турнир и проводить бои.
- `tournaments.rating_id` — опциональная ссылка на `https://rating.chgk.info/tournament/<rating_id>` (целое число). Если NULL — турнир не размечен в рейтинге ЧГК.
