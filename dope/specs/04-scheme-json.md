...
- На текущей итерации импорт пересоздает дефолтную игру дефолтного турнира в SQLite: схему, этапы, бои, слоты, площадки. (В следующих итерациях /api/import будет принимать `tournament_id` и `game_code` явно.)
