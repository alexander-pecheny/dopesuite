...
Not a thing a [[Representative]] creates: every [[Слот]] has one, reached by an unguessable link the Representative copies and posts. It starts closed — the link exists from the Slot's first moment, and a Slot that registered itself would take Заявки before anyone had picked the турнир — and the Representative opens it by hand, at once or at a set time, and closes it again the same way. Anyone can open the link and read what and when; filing a [[Заявка]] needs a Telegram login. A closed registration still shows a user their own заявка.
