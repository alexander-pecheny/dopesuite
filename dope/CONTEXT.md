...
**Зачёт (Division)**:
A standings a team competes in. One tournament may run several зачёты at once on the same questions. At an Открытый чемпионат Польши, Polish teams compete for the national medals while every team competes for the festival's. «Вне зачёта» is itself a зачёт: usually adult teams playing a school or student championship for fun. A Division is the set of teams carrying one [[Flag]], ranked among themselves. Places are dealt again inside it, while whatever a question is worth (ОД's рейтинг) is still counted over everyone, because everyone played it. «Все» is the общий зачёт. A viewer looks at one Division at a time, and a team with two Flags stands in both. Source sheets normally mark a team's зачёт in a column of its own. Вне-зачёта teams are shown below the ranked ones, in full and not greyed out. Marking a team out of a Division is not an [[Отказ]].

**Flag (Флаг)**:
A mark a fest team carries; the word is the rating site's. Examples are «Школьная команда», «Студенческая команда», and regional ones such as «Европа». It comes with the roster import or an organizer types it, and every distinct Flag among a fest's teams offers a [[Зачёт (Division)]] to look at. The app does not decide which Flags deserve one. It is known by its short name (Школ, Студ, Е) wherever it is shown or linked.
_Avoid_: the country-flag emoji on the ОД Экран. That is a decoration derived from the city, not a Flag.
