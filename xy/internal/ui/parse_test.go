...
func TestParse_EscapesInStrings(t *testing.T) {
	doc, err := Parse("t.xui", []byte(`text "a\"b\\c\nd"`+"\n"))
	if err != nil {
		t.Fatalf("Parse: %v", err)
	}
...
func TestParse_NestedParens(t *testing.T) {
	doc, err := Parse("t.xui", []byte(`hint (strong (code "go"))`+"\n"))
	if err != nil {
		t.Fatalf("Parse: %v", err)
	}
...
func TestParse_BareAndValueAttrsPreserveOrder(t *testing.T) {
	doc, err := Parse("t.xui", []byte(`textfield required autofocus name="u"`+"\n"))
	if err != nil {
		t.Fatalf("Parse: %v", err)
	}
	elem := doc.Nodes[0].(*Element)
