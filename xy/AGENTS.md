...
web/ts/                strict-TS ES-module sources; built by `just build-web` into
    ...
    suggest.ts         xy's choices for the shared dropdown (zoneChoices/townChoices);
                       `autocomplete` itself lives in dopeuikit/assets/ts/suggest.ts,
                       bundled into this one module so the ESM build's import stays
                       inside dist
