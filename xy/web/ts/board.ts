...
async function cardAttachments(cardId: number, refresh = false): Promise<NamedAttachment[]> {
  ...
  const out = await Promise.all(atts.map(async (att) => {
    let name = "";
    try { name = await xyCrypto.decField(mustDK(), att.filename_enc); } catch (_) {}
    return { ...att, name };
  }));
...
async function attachmentUrl(att: Attachment): Promise<string> {
  ...
  const plain = await xyCrypto.decBytes(mustDK(), cipher);
...
const cardDetail = createCardDetail({
  ...
  questionNumberFor,
  cleanupTestLabels,
  preview: { renderPreviewCard, resolveImages, imageRefs, fillPreviewImages, previewList },
  attachments: {
    load: loadAttachments,
    imageNames: () => cardImageNames,
    clearImageNames: () => { cardImageNames = []; },
  },
...
timeline = createTimeline({
  ...
  attachments: { url: attachmentUrl, download },
});
...
document.addEventListener("paste", (e) => {
  ...
  if (cardDetail.openCardId() == null || cardOverlay.hidden) return;
