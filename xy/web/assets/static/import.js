...
function collectActions(actions, history) {
...
const emptyHistory = () => ({ comments: new Map(), descEdits: new Map() });
...
async function fetchHistory(token, boardId) {
...
async function runImport(source, name, pass) {
  ...
  let done = 0;
  const tally = { comments: 0, edits: 0, attachments: 0 };
  const errors = [];
  for (const l of openLists) {
...
async function importNormalCard(boardId, listId, c, rank, enc, labelMap) {
  ...
  const body = { description_enc: await enc(desc), rank, kind };
  if (alias) body.alias_enc = await enc(alias);
  const res = await jpost(`/api/lists/${listId}/cards`, body);
...
// recorded.
async function importCardExtras(card, c, source, enc, dk, tally, errors) {
  ...
  if (events.length) {
    ...
    try {
      ...
      errors.push(`«${c.name || c.id}» история: ${e.message}`);
    }
  }
