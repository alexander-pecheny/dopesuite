...
Three bounded contexts, one per app/UI module (`dopecore` is a shared platform layer with no domain glossary). Read the `CONTEXT.md` of whichever you're working in; root `docs/adr/` holds system-wide decisions, `<module>/docs/adr/` module-scoped ones.
